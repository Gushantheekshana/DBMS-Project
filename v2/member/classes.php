...
<div class="table-wrap"><table><thead><tr><th>Class</th><th>Trainer</th><th>Schedule</th><th>Places</th><th>Action</th></tr></thead><tbody><?php foreach ($classes as $class): ?><tr><td><strong><?= e($class['Name']) ?></strong><br><small><?= e($class['Location'] ?? '—') ?></small></td><td><?= e($class['TrainerName'] ?? 'TBA') ?></td><td><?= e($class['StartsAt']) ?> – <?= e($class['EndsAt']) ?></td><td><?= (int) $class['EnrolledCount'] ?>/<?= (int) $class['Capacity'] ?></td><td><?php if ($class['EnrollmentStatus']): ?><?= e($class['EnrollmentStatus']) ?><?php else: ?><form method="post"><?= csrf_field() ?><input type="hidden" name="class_id" value="<?= (int) $class['GymClassID'] ?>"><button class="btn btn-primary">Request enrollment</button></form><?php endif; ?></td></tr><?php endforeach; ?><?php if (!$classes): ?><tr><td colspan="5">No upcoming classes.</td></tr><?php endif; ?></tbody></table></div>
