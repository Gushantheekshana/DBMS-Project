...
`php v2/cli/seed-demo.php` is available only in `APP_ENV=local`. It retains the original one-per-role demonstration accounts and adds five connected fictional scenarios numbered `101` through `105`. Each added scenario includes a member and trainer account/profile, plan, membership, payment, completed class, enrollment, attendance record, and completed gym visit. The catalog uses six realistic LKR plans: Basic Fitness, Standard Plus, Premium, VIP Elite, Family Plan, and Student Plan. Unlimited class access is stored as a `NULL` class limit. Every email ends in `example.test`, every account uses `Demo@12345`, and only the existing `ADMIN`, `RECEPTIONIST`, `TRAINER`, and `MEMBER` roles are used. Re-running updates reserved plan rows and demo relationships in place without truncating tables or modifying unrelated plans or memberships.
