...
<section class="stats-grid">
  ...
  <article class="stat-card"><span>Membership</span><strong><span class="status-badge status-<?= e(strtolower($member['MembershipStatus'] ?? 'none')) ?>"><?= e(ucwords(strtolower($member['MembershipStatus'] ?? 'NONE'))) ?></span></strong><small><?= e($member['PlanNameSnapshot'] ?? 'Choose a plan') ?></small></article>
  <article class="stat-card"><span>Ends</span><strong><?= e($member['EndsOn'] ?? '—') ?></strong><small><?= count($upcoming) ?> upcoming</small></article>
</section>
<section class="panel"><h2>Upcoming classes</h2><div class="table-wrap"><table><caption class="sr-only">Your upcoming classes</caption><thead><tr><th>Class</th><th>Starts</th><th>Location</th><th>Status</th></tr></thead><tbody>
<?php foreach ($upcoming as $class): ?><tr><td><?= e($class['Name']) ?></td><td><?= e($class['StartsAt']) ?></td><td><?= e($class['Location'] ?? '—') ?></td><td><span class="status-badge status-<?= e(strtolower($class['Status'])) ?>"><?= e(ucwords(strtolower($class['Status']))) ?></span></td></tr><?php endforeach; ?>
