...
?>
<section class="panel"><h2>Class attendance</h2><div class="table-wrap"><table><caption class="sr-only">Your class attendance history</caption><thead><tr><th>Class</th><th>Starts</th><th>Status</th><th>Checked in</th></tr></thead><tbody><?php foreach ($classAttendance as $row): $attendanceStatus = $row['Status'] ?? 'NOT MARKED'; ?><tr><td><?= e($row['Name']) ?></td><td><?= e($row['StartsAt']) ?></td><td><span class="status-badge status-<?= e(strtolower(str_replace(' ', '-', $attendanceStatus))) ?>"><?= e(ucwords(strtolower($attendanceStatus))) ?></span></td><td><?= e($row['CheckedInAt'] ?? '—') ?></td></tr><?php endforeach; ?><?php if (!$classAttendance): ?><tr><td colspan="4">No class attendance records.</td></tr><?php endif; ?></tbody></table></div></section>
<section class="panel"><h2>Gym visits</h2><div class="table-wrap"><table><caption class="sr-only">Your gym visit history</caption><thead><tr><th>Checked in</th><th>Checked out</th><th>Notes</th></tr></thead><tbody><?php foreach ($visits as $row): ?><tr><td><?= e($row['CheckedInAt']) ?></td><td><?= e($row['CheckedOutAt'] ?? 'Still inside') ?></td><td><?= e($row['Notes'] ?: '—') ?></td></tr><?php endforeach; ?><?php if (!$visits): ?><tr><td colspan="3">No gym visits.</td></tr><?php endif; ?></tbody></table></div></section>
